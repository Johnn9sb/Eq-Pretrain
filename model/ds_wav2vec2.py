...
class Wav2Vec2Config(FairseqDataclass):
    ...
    conv_feature_layers: str = field(
        ...
        default = "[(64,11,2)] + [(128,5,1)] + [(256,5,1)] + [(512,7,2)]",
    ...
    feature_grad_mult: float = field(
        ...
        default=0.1, metadata={"help": "multiply feature extractor var grads by this"}
    )
...
@register_model("wav2vec2test", dataclass=Wav2Vec2Config)
...
class ConvFeatureExtractionModel(nn.Module):
    ...
    def make_seq(
        ...
        if stride == 2:
            DSconv1 = nn.Sequential(
                nn.Conv1d(in_d, in_d, kernel_size=1, bias=False),
                nn.Conv1d(
                    in_channels=in_d, 
                    out_channels=in_d, 
                    kernel_size=k+4, 
                ...
                nn.Conv1d(in_d, dim, kernel_size=1, bias=False),
                nn.Dropout(p=dropout),
                Fp32GroupNorm(dim, dim, affine=True),
                nn.GELU(), 
            )
            DSconv2 = nn.Sequential(
                nn.Conv1d(in_d, in_d, kernel_size=1, bias=False),
                nn.Conv1d(
                    in_channels=in_d, 
                    out_channels=in_d, 
        ...
        return DSconv1,DSconv2,DSconv3
        ...
        for i, cl in enumerate(conv_layers):
           ...
           #(512, 11, 2    )
            ...
            in_d = dim
            self.out_proj = nn.Conv1d(in_channels=1536, out_channels=512,kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        for i, cl in enumerate(self.conv_layers):
            ssert len(cl) == 3, "invalid conv definition: " + str(cl)
            (dim, k, stride) = cl

            DSconv1,DSconv2,DSconv3 = self.make_seq(
                n_in=in_d,
                n_out=dim,
                k=k,
                stride=stride,
