...
class STEAD_noise(BenchmarkDataset):
    ...
    def _download_dataset(self, writer: WaveformDataWriter, basepath=None, **kwargs):
        download_instructions = (
            ...
            "Provide the locations of the STEAD unpacked files (chunk1.csv and chunk1.hdf5) in the "
        ...
        if not (basepath / "chunk1.csv").is_file():
            raise ValueError(
                "Basepath does not contain file chunk1.csv. " + download_instructions
            )
        if not (basepath / "chunk1.hdf5").is_file():
            raise ValueError(
                "Basepath does not contain file chunk1.hdf5. " + download_instructions
            )

        # Copy metadata and rename columns to SeisBench format
        metadata = pd.read_csv(basepath / "chunk1.csv")
        ...
        # set split: 80% for training, 10% for validation, 10% for testing
        train_split = int(len(metadata) * 0.8)
        valid_split = int(train_split + len(metadata) * 0.1)

        with h5py.File(basepath / "chunk1.hdf5") as f:
            gdata = f["data"]
            for idx, row in metadata.iterrows():
                ...
                if idx < train_split:
                    row["split"] = "train"
                elif idx >= train_split and idx < valid_split:
                    row["split"] = "dev"
                else:
                    row["split"] = "test"
