...
class SlidingWindowWithLabel(FixedWindow):
    '''
    將原本的 sliding window 方法再加上對應 window 的 label 結果
    '''
    ...
    def __call__(self, state_dict):
        ...
        label = state_dict['y'][0]
        
        window_outputs = []
        window_labels = []
        for i in range(waveforms.shape[1]):
            if i*timestep+windowlen >= waveforms.shape[1]:
                break
                
            window_outputs.append(waveforms[:, i*timestep:i*timestep+windowlen])
            window_labels.append(label[:, i*timestep:i*timestep+windowlen])
            
        
        x_out = np.stack(window_outputs, axis=0)
        y_out = np.stack(window_labels, axis=0)
    
        state_dict['X'] = (x_out, metadata)
        state_dict['y'] = (y_out, state_dict['y'][1])
