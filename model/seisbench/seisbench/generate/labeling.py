...
class RED_PAN_label:
    def __init__(self, key="X", p_label_columns="trace_p_arrival_sample", s_label_columns="trace_s_arrival_sample", p_win=40, s_win=60, length=3000):
        if isinstance(key, str):
            self.key = (key, key)
        else:
            self.key = key

        self.p_label_columns = p_label_columns
        self.s_label_columns = s_label_columns
        self.p_win = p_win
        self.s_win = s_win
        self.length = length
    ...
    def __call__(self, state_dict):
        ...
        try:
            ...
            ptime = metadata[self.p_label_columns]
            stime = metadata[self.s_label_columns]
            
            trc_psn, trc_mask = gen_target(ptime=int(ptime), stime=int(stime), length=waveforms.shape[-1])
            
            # concat trc_psn, trc_mask into waveforms
            waveforms = np.concatenate((waveforms, trc_psn, trc_mask), axis=0)
        ...
        # noise
            ...
            waveforms = np.concatenate((waveforms, np.zeros((2, waveforms.shape[-1])), np.ones((1, waveforms.shape[-1])), np.zeros((1, waveforms.shape[-1])), np.ones((1, waveforms.shape[-1]))), axis=0)
            state_dict[self.key[1]] = (waveforms, metadata)
